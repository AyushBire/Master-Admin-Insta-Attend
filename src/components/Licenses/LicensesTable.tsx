import { RefreshCw, Settings2, Ban, CheckCircle2, Building2 } from "lucide-react";
...
interface LicensesTableProps {
  ...
  onModify: (license: License) => void;
  onToggleCancel: (license: License) => void;
}

export default function LicensesTable({ licenses, onRenew, onModify, onToggleCancel }: LicensesTableProps) {
            ...
            licenses.map((lic) => {
              ...
              return (
                <tr key={lic.id} className="table-row">
                  ...
                  <td className="table-cell">{lic.billingCycle}</td>
                  <td className="table-cell">{formatDate(lic.startedAt)}</td>
                  <td className="table-cell">
                    <p>{formatDate(lic.expiresAt)}</p>
                    <p className="text-xs text-text-muted">{formatDaysRemaining(lic.expiresAt, lic.cancelled)}</p>
                  </td>
                  <td className="table-cell">
